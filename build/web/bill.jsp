    ...
    <body>
        <%
            ...
            try {
                ...
                ResultSet rs = st.executeQuery("select sum(total) from cart_jsp where email='" + email + "' and status='bill'");
                while (rs.next()) {
                    total = rs.getInt(1);
                }
                ResultSet rs2 = st.executeQuery("select *from users_jsp inner join cart_jsp where cart_jsp.email='" + email + "' and cart_jsp.status='bill'");
                while (rs2.next()) {
        %>
        <h3>Online shopping Bill (ARIJIT GUHA)</h3>
        <hr>
        ...
        <table id="customers">
            ...
            <%
                ResultSet rs1 = st.executeQuery("select *from cart_jsp inner join product_jsp where cart_jsp.product_id=product_jsp.id and cart_jsp.email='" + email + "' and cart_jsp.status='bill'");
                while (rs1.next()) {
                    sno = sno + 1;
